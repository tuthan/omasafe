...
pub fn source_fingerprint(src: &Path) -> std::io::Result<(String, Vec<PathBuf>)> {
    ...
    feed(&mut hasher, b"omasafe.analyzer-logic.v1");
    for path in &files {
...
/// Cargo must watch these directories as well as the files currently present:
/// watching only existing files misses a newly-created module until some
/// other input happens to trigger the build script.
