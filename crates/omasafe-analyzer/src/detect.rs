    ...
    pub fn rendered_findings(&self) -> Vec<RenderedFinding> {
        ...
        for result in &self.results {
            ...
            rendered.push(RenderedFinding {
                ...
                line: result.line(),
                evidence: truncate_bytes(result.semantic_value(), MAX_EVIDENCE_BYTES_PER_RESULT),
        ...
        rendered.sort_by(|a, b| {
            (&a.relative_path, &a.rule_id, a.line, &a.evidence).cmp(&(
                &b.relative_path,
                &b.rule_id,
                b.line,
                &b.evidence,
...
pub fn analyze_inventory(
    ...
    for index in 0..inventory.entries.len() {
        ...
        let analyzable = matches!(entry.kind, PayloadKind::Qml | PayloadKind::JavaScript);
        if !analyzable
            || !matches!(
                entry.coverage_state,
                CoverageState::Unsupported | CoverageState::Unreferenced
            )
        ...
        let mut outcome = match entry.kind {
            PayloadKind::Qml => analyze_qml_source(&source),
            _ => analyze_javascript_source(&source),
        };
        ...
        inventory.entries[index].coverage_state = if outcome.parse_degraded {
...
fn occurrence(
    ...
    let covering_rule = match capability {
        ...
        Capability::PersistenceScheduling => Some("oma.qml.persistence-scheduling"),
        _ => None,
    }
    ...
    CapabilityOccurrence {
        ...
        explanation: covering_rule
            ...
            .unwrap_or_else(|| "capability observed; dedicated rule lands in S4".to_owned()),
        review_guidance: covering_rule
            .map(|definition| definition.review_guidance.to_owned())
            .unwrap_or_else(|| "Review manually; automated coverage arrives with S4".to_owned()),
    }
}
...
/// The executable prefix of a source line: everything before the first
/// `//` that is not inside a quoted string. Quoted text like
/// `"https://host"` survives; trailing comments never reach detection.
...
mod ast {
    ...
    fn number_of(node: tree_sitter::Node) -> u32 {
